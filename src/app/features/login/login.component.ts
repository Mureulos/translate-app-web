...
export class LoginComponent {
  ...
  onSubmitLogin(): void {
    if (this.loginForm.valid) {
      ...
      this._authService
        .login(loginRequest)
        .pipe(finalize(() => this._utilsService.hide()))
        .subscribe({
        ...
        .subscribe({
          next: (response: LoginResponse) => {
            ...
            this._messageService.add({
              severity: 'success',
              summary: 'Sucesso',
              detail: 'Conta criada e login realizado com sucesso!',
            });
            this._router.navigate(['']);
          },
          error: (error) => {
            this._messageService.add({
              severity: 'error',
              summary: 'Erro',
              detail: error.message || 'Falha ao registrar ou efetuar login.',
            });
          },
        });
    } else {
